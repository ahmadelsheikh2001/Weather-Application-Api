...
const OneDayWeather = ({ weather, loading, error }) => {
  ...
  const [alldata, setAllData] = useState([
    { input1: value1, input2: value2, input3: value3, input4: state }
  ])
  // ///////////////////
  ...
  const [seaLevelPressure, setSeaLevelPressure] = useState("");
  const [WetBulb, setWetBulb] = useState(null);

  useEffect(() => {
    if (weather) {
      setDryTemperature(weather.current.feelslike_c - 4);
  ...
  function calculateWeatherValues(weather) {
    try {
      ...
      if (
        Number(wB) == Number(temp) ||
        Math.abs(parseInt(wB) - parseInt(temp)) < 1
      ) {
        humitityRH = 100;
      }
  ...
  const weatherValuess =
    !loading && !error && weather && weatherValues
      ? Computevprh(
        dryTemperature ? dryTemperature : weather.current.feelslike_c,
        weatherValues.wB
      )
      : {};
      ...
      {!loading && !error && (
        <>
          <div className="  mt-3 row d-flex justify-content-center flex-nowrap">
            <div className="col mb-3 pb-3 bg-light me-1 d-flex justify-content-start align-items-start flex-column shadow-sm rounded-3 p-3"
              style={{
                height: "fit-content",
                maxWidth: "22em",
              }}
            >
              {
            //     <span className="h6 fw-bolder text-black span-bg">
            //   Country: {weather && weather.location.country}
            // </span>
          
              // <span className="h6 fw-bolder text-black span-bg">
              //   City: {weather && weather.location.name}
              // </span>
              }
              <span className="h6 fw-bolder text-black span-bg">
                Date:{" "}
              ...
              <div className="d-flex flex-column justify-content-start align-items-start w-100">
                <div className="form-floating mb-3 me-2 w-100 input-bg">
                  <input
                    type="text"
                    className="form-control input-shadow dry-inp"
                    id="dry-temprature"
                    // ref={dryTemp}
                    autoComplete="off"
                    onChange={onDryTemperatureChange}
                  />
                  <label htmlFor="dry-temprature">Dry Temperature</label>
                </div>
                <div className="form-floating mb-3 w-100 input-bg">
                  <input
                    className="form-control input-shadow wet-inp"
                    id="wet-bulb"
                    type="text"
                    // value={weatherValues.wB}
                    onChange={onWetBulbChange}
                    autoComplete="off"
                  />
              ...
              <div className="w-100 border-bottom border-1 mb-3"></div>
              <span className="h6 fw-bolder text-dark span-bg">
              ...
              <span className="h6 text-dark fw-bolder span-bg">
                Wind Gust:{" "}
                {weather &&
                  weather.forecast.forecastday[0].hour.map((data) =>
                    data.time.toString().split(" ")[1].split(":")[0] ===
                      current_hour
                      ? Math.round(weather && weather.current.gust_kph / 2) +
                      " KT"
                      : ""
                  )}
              </span>
              ...
              <div className="w-100 border-bottom border-1 mb-3"></div>
              <span
                className="h6 fw-bolder text-dark span-bg"
                style={{ display: "none" }}
              >
                Visibility: {weather && weather.current.vis_km + " KM"}
              </span>
              <span className="h6 fw-bolder text-danger span-bg">
                Dew Point: {TD > 0 ? TD : weatherValuess.tdtdtd} °
              </span>
              <span className="h6 fw-bolder text-danger span-bg">
                Humidity: {RH > 0 ? RH : weatherValuess.rhhh} %
              </span>
              <span className="h6 fw-bolder text-danger span-bg">
                Actual Vapor Pressure: {VP > 0 ? VP : weatherValuess.vppp}
              </span>
              <span className="h6 fw-bolder text-dark span-bg">
                Vertical Temprature: {weatherValues.virtualTemperatureC + " C°"}
              </span>
              <div className="w-100 border-bottom border-1 mb-3"></div>
              ...
              {/*}</span>*/}
              <span className="h6 fw-bolder text-dark span-bg">
                Saturated Vapor Pressure: {weatherValues.saturatedVaporPressure}
              </span>
              <span className="h6 fw-bolder text-dark span-bg">
                Saturated Mixing Ratio: {weatherValues.saturatedMixingRatio}
              </span>
            </div>
            <div className="col mb-2 bg-light me-1 d-flex justify-content-start flex-column shadow-sm rounded-3 p-3">
              <Graphs />
            </div>
            <div className="col mb-2 bg-light me-1 d-flex justify-content-start flex-column shadow-sm rounded-3 p-3">
              <Inputs sea={seaLevelPressure} />
              {/* <input type="text" seaValue = {seaLevelPressure} setSeaVlue={setSeaLevelPressure} /> */}
            </div>
          </div>
        </>
      )}
