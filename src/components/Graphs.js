...
export default function Graphs() {
        ...
        const fetchWeatherData = async () => {
            ...
            try {
                const response = await axios.get(
                    ...
                    // `https://api.weatherapi.com/v1/forecast.json?key=b7c9a474596846538a8130316230608&q=${selectedCity}&days=14`,
                    {
                        params: {
    ...
    if (!weather || !weather.forecast.forecastday) {
        return <div style={{ marginTop: 50, fontSize: 18 }}>No weather data available.</div>;
    }
    const boldStyle = {
        fontWeight: 'bold',
    };
        ...
        weather.forecast.forecastday[0].hour.forEach((hourData) => {
            ...
            if (time.getMonth() <= 5 || (time.getMonth() >= 10 && time.getDay() >= 1))
                data.push({
                    ...
                    gustSpeed: Math.round(hourData.gust_mph) / 2,
                });
            else
                data.push({
                    SeaLevel: selectedCity === "Baghdad" ? hourData.pressure_mb - 4.2 : hourData.pressure_mb,
    ...
    return (
        <>
            ...
            <div className='graph-container' style={{ display: 'none' }}>
                {/* Third Graph */}
                <LineChart
                    width={900}
                    height={370}
